...
def section_graph(lane_markings: list[float],
    ...
    for j, lmi in enumerate(lane_markings):
        ...
        if j in (0, n_markings - 1):
            node_cls = np.ones_like(x_l) * EdgeType.ROAD_BORDER.value
        else:
            node_cls = np.ones_like(x_l) * EdgeType.LINE_THIN_DASHED.value
    ...
    for j in range(n_markings):
        ...
        if j in (0, n_markings - 1):
            edge_cls = np.ones((len(edges[0]), 1)) * EdgeType.ROAD_BORDER.value
        else:
            edge_cls = np.ones((len(edges[0]), 1)) * EdgeType.LINE_THIN_DASHED.value
...
if __name__ == "__main__":
    ...
    for i in range(plot_data['map_point', 'to', 'map_point']['edge_index'].shape[1]):
        ...
        COLOR = 'k' if plot_data['map_point', 'to', 'map_point']['type'][i] == EdgeType.ROAD_BORDER.value else 'grey'
